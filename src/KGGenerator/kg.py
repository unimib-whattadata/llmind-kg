...
INCLUSION_PROP = URIRef("http://id.who.int/icd/schema/inclusion")
EXCLUSION_PROP = URIRef("http://id.who.int/icd/schema/exclusion")
HAS_DIAGNOSTIC_REQUIREMENTS_PROP = URIRef(
    f"http://icd_kg/6/ontology/hasDiagnosticRequirements")
...
class HierarchicalRepresentation:
    ...
    def add_entities(self, graph: Graph):
        ...
        for node in tqdm(all_nodes):
            ...
            current_entity_id = URIRef(
                f"{self.entity_uri_base}/{current_entity_name.replace(' ', '_')}")
            # add label
            ...
            for parent in parents:
                ...
                parent_id = URIRef(
                    f"{self.ontology_uri_base}/{parent_name.replace(' ', '_')}")
        ...
        for node in tqdm(all_nodes):
            ...
            current_class_id = URIRef(
                f"{self.entity_uri_base}/{current_class_name.replace(' ', '_')}")
            for exclusion in current_exclusions:
                ...
                if exclusion_node is None:
                    ...
                    exclusion_node_id = URIRef(
                        f"{self.entity_uri_base}/{exclusion_node_name.replace(' ', '_')}")
                    if any(graph.triples((current_class_id, None, None))):
                        graph.add((current_class_id,
        ...
        for entity in tqdm(all_entities):
            ...
            entity_uri = URIRef(
                f"{self.entity_uri_base}/{entity_name.replace(' ', '_')}")
            # Add Symptoms
    ...
    def create_ontology(self, add_entity: bool = False):
        ...
        graph.add((URIRef(disease_uri), OWL.equivalentClass, SCHEMA.Disease))
        ...
        graph.add((URIRef(drug_uri), OWL.equivalentClass, SCHEMA.DrugClass))
        ...
        graph.add((URIRef(diagnostic_criteria_uri), RDF.type, OWL.Class))

        # Add relations
        graph.add((URIRef(drug_uri), SCHEMA.drug, URIRef(disease_uri)))
        graph.add((URIRef(disease_uri), SCHEMA.signOrSymptom, URIRef(symptom_uri)))
        ...
        for node in tqdm(all_nodes):
            ...
            current_class_id = f"{self.ontology_uri_base}/{current_class_name.replace(' ', '_')}"
            ...
            graph.add((URIRef(current_class_id), RDF.type, OWL.Class))
            # add description
            ...
            for child in node["child"]:
                ...
                child_id = f"{self.ontology_uri_base}/{child_name.replace(' ', '_')}"
        ...
        for node in tqdm(all_nodes):
            ...
            for exclusion in current_exclusions:
                ...
                if exclusion_node is None:
                    ...
                    exclusion_node_name = self.clean_name(
                        exclusion_node["title"]["@value"])
                    exclusion_node_id = f"{self.ontology_uri_base}/{exclusion_node_name.replace(' ', '_')}"
...
HierarchicalRepresentation().create_ontology(add_entity=False)
